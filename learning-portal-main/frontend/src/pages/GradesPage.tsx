...
const GradesPage: React.FC = () => {
  ...
  const [editForm, setEditForm] = useState({ grade: '5', comment: '', date: '' });
  const { user } = useAuth();
  const isAdminLike = user?.role === 'admin' || user?.role === 'owner';
  const canEditGrade = (g: Grade) =>
    isAdminLike || (user?.role === 'trainer' && g.trainer_id === user?.id);
  ...
  const handleSaveEditGrade = async () => {
    ...
    const g = parseFloat(editForm.grade);
    if (Number.isNaN(g) || g < 0 || g > 5) {
      setError('Оценка должна быть от 0 до 5');
      return;
    }
    setError('');
    try {
      ...
      setInfo('Оценка обновлена');
      setEditingGrade(null);
      if (historyStudentId) await handleHistoryStudentChange(historyStudentId);
      else await loadGrades();
    } catch (err: any) {
      setError(err.response?.data?.detail || 'Ошибка обновления оценки');
    }
  };
  ...
  return (
    <Layout>
        ...
        <Paper sx={{ p: 2, mb: 2 }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            ...
            <TextField
              ...
              helperText="Можно указать прошедшую дату"
            />
      ...
      <Dialog open={!!editingGrade} onClose={() => setEditingGrade(null)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          <Button onClick={() => setEditingGrade(null)}>Отмена</Button>
          <Button variant="contained" onClick={handleSaveEditGrade}>
            Сохранить
          </Button>
        </DialogActions>
      </Dialog>
