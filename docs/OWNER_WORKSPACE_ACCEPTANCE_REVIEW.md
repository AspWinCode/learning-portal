...
| Portable admin bundle import/export | DONE | Full owner-workspace settings bundle can be exported as a versioned metadata envelope, copied to clipboard, previewed before import, imported back through normalized server-side application with backward-compatible raw-bundle support, stored as named rollback snapshots in the admin UI, reviewed before apply with changed-section preview, protected with an optional automatic safety snapshot, managed through snapshot metadata editing and duplication actions, surfaced with inline current-vs-snapshot diff visibility plus filtering/sorting in the snapshot list, compared in the review dialog against either current state or another snapshot, previewed as raw JSON directly from the snapshot list, and cleaned up through explicit delete confirmation. |
