...
// ============================================================================
// PATCH: Bypass browser disk cache for resources with permission issues
// ============================================================================
self.addEventListener('fetch', event => {
  ...
  const needsBypass = (
    ...
    url.includes('/res/') ||
    url.includes('/CppPlatform/') ||
    url.includes('/JsPlatform/')
  );
  if (needsBypass) {
    event.respondWith(
...
if (workbox) {
  ...
  const VersionMetadata = {"version":"5.6.257","gitHash":"unknown-hash","versionWithHash":"5.6.257-unknown-hash"};
