...
export const studentPortalAdminApi = {
  ...
  }): Promise<StudentCourseAccessOut> => {
...
export const financeApi = {
  ...
  applyTransactionSplit: async (
    transactionId: number,
    payload: { splits: { student_id: number; amount: number }[] }
  ): Promise<FinanceLedgerBankRow> => {
    const response = await api.post(`/api/finance/transactions/${transactionId}/apply-split`, payload);
    return response.data;
  },
...
export const groupsApi = {
  ...
  updateStudent: async (groupId: number, studentId: number, data: { custom_duration_minutes?: number | null; schedule_ids?: number[] }): Promise<void> => {
...
export const salesApi = {
  ...
  listPublicCities: async (): Promise<string[]> => {
    const response = await api.get('/api/sales/public/cities');
    return response.data;
  },
  listPublicClasses: async (): Promise<string[]> => {
    const response = await api.get('/api/sales/public/classes');
    return response.data;
  },
  listPublicLeadSources: async (): Promise<string[]> => {
    const response = await api.get('/api/sales/public/lead-sources');
    return response.data;
  },
  submitIndividualLessonsQuestionnaire: async (payload: {
    child_full_name: string;
    birth_date: string;
    child_phone: string;
    student_email: string;
  ...
  submitProgrammerQuestionnaire: async (payload: {
    child_full_name: string;
    birth_date: string;
    child_phone: string;
    student_email: string;
...
export const tasksApi = {
  ...
  bulkCreateSubtasks: async (taskId: number, titles: string[]): Promise<{ created: number; subtasks: import('../types').TaskSubtaskResponse[] }> => {
    const response = await api.post(`/api/tasks/${taskId}/subtasks/bulk`, { titles });
    return response.data;
  },
