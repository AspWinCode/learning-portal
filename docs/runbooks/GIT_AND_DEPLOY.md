...
### 4. Если в ошибке «ответ: 404» при правильном URL (например https://ваш-домен/api/sales/leads)

404 приходит либо от Caddy (статики), либо от бэкенда. Проверьте по шагам на сервере:

**А) Открыть в браузере:** `https://ваш-домен/api/health`  
- Если **200** и `{"status":"ok"}` — прокси до бэкенда работает, 404 по `/api/sales/leads` отдаёт сам бэкенд (маршрут или метод).  
- Если **404** — запросы `/api/*` не доходят до бэкенда, смотрите конфиг Caddy и пересоберите контейнер **web**.

**Б) Прямой запрос к бэкенду с сервера (в папке проекта):**
```bash
# с токеном (подставьте свой токен из localStorage после входа на сайт)
curl -s -o /dev/null -w "%{http_code}" -H "Authorization: Bearer ВАШ_ТОКЕН" http://127.0.0.1:8000/api/sales/leads
```
- **200** — бэкенд отвечает, проблема в прокси/Caddy.  
- **401** — нормально без валидного токена.  
- **404** — бэкенд не находит маршрут (см. ниже пересборку backend и проверку маршрутов).

**В) Проверить, зарегистрирован ли GET /api/sales/leads (одной командой, без скрипта):**
```bash
docker compose exec backend python -c "
from app.main import app
import json
o = app.openapi()
for path, methods in (o.get('paths') or {}).items():
    if 'leads' in path:
        for m in methods:
            print(m.upper(), path)
"
```
Если в выводе нет строки `GET /api/sales/leads` — пересоберите backend (см. ниже).

После правок Caddyfile пересоберите и поднимите **web**:
```bash
