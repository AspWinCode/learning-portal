...
| Portable admin bundle import/export | DONE | Full owner-workspace settings bundle can be exported as a versioned metadata envelope, copied to clipboard, previewed before import, imported back through normalized server-side application with backward-compatible raw-bundle support, and stored as named rollback snapshots in the admin UI. |
