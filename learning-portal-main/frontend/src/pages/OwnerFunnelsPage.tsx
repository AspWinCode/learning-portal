...
/** Этапы воронки Мероприятия, при переходе на которые показывается popup */
...
const OwnerFunnelsPage: React.FC = () => {
  ...
  const loadCities = useCallback(async () => {
    try {
      ...
      setError(extractApiError(err, 'Не удалось загрузить города'));
    }
  }, []);
  ...
  const handleFunnelChange = (funnelId: string) => {
    ...
    // При выборе «Мероприятия» сразу открываем popup создания воронки (название + даты)
  ...
  const handleAddSchools = async () => {
    ...
    try {
      ...
      if (result.added > 0) {
        setError(null);
        // Можно показать успех: добавлено result.added из result.total_in_city
      }
    } catch (err: any) {
  ...
  return (
    <Layout>
                ...
                <Box sx={{ pt: 1, borderTop: 1, borderColor: 'divider' }}>
                  ...
                  <Stack spacing={1.5}>
                    ...
                    {cardDetailSchool.friendship_degree && (
                      <Box>
                        <Typography variant="caption" color="text.secondary">Степень дружелюбия</Typography>
                        <Typography variant="body2">{cardDetailSchool.friendship_degree}</Typography>
                      </Box>
                    )}
                    <Box>
                      <Typography variant="caption" color="text.secondary">Этап в B2B</Typography>
              ...
              {isEventsFunnel && cardDetailItem.card_data && (
                <>
                  ...
                  {(cardDetailItem.card_data as OwnerFunnelCardData).reply_comment && (
                    <Box>
                      <Typography variant="caption" color="text.secondary">Ответное письмо (комментарий)</Typography>
      ...
      <Dialog open={addSchoolsOpen} onClose={() => setAddSchoolsOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>
            В воронку будут добавлены все B2B школы из выбранного города (в этап «Новые»). Уже добавленные школы пропускаются.
          </Typography>
        </DialogContent>
        <DialogActions>
          ...
          {isEventsFunnel ? (
            <Stack spacing={2} sx={{ mt: 1 }}>
              ...
              <TextField
                ...
                placeholder="Например: 15.03.2026 – 20.03.2026"
              />
            </Stack>
          ) : (
            <>
      ...
      {/* Popup при переходе на этап воронки Мероприятия */}
      <Dialog
        open={!!stagePopup}
      ...
      >
        <DialogTitle>
          {stagePopup?.kind === 'contact' && 'Контакт найден'}
          {stagePopup?.kind === 'reply' && 'Получили ответное письмо'}
          {stagePopup?.kind === 'meeting' && 'Договорились на встречу'}
          {stagePopup?.kind === 'trip' && 'Договорились на поход'}
          {stagePopup?.kind === 'leads' && 'Собрали лидов'}
        </DialogTitle>
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            {stagePopup?.kind === 'reply' && (
              <TextField
                ...
                multiline
                rows={3}
                placeholder="Текст ответа или заметка"
              />
            )}
            {stagePopup?.kind === 'meeting' && (
              <TextField
                ...
                placeholder="Например: 25.03.2026 в 14:00"
              />
            )}
