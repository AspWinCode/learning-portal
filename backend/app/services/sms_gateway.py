...
Отправка SMS через API приложения; история и шаблоны хранятся в CRM backend.
"""
...
# Настройки из env (не доступны на frontend)
...
def is_configured() -> bool:
    """Проверка, что gateway настроен (URL и токен)."""
...
def send_sms(phone: str, message: str) -> Tuple[bool, Optional[str], Optional[str]]:
    ...
    if not base_url or not token:
        logger.warning("sms_gateway_error: SMS Gateway not configured (missing BASE_URL or TOKEN)")
    ...
    url = f"{base_url}/3rdparty/v1/messages"
    headers = {
    ...
    if resp.status_code in (200, 201, 202):
        try:
            ...
            if isinstance(data, dict):
                gateway_id = data.get("id") or data.get("messageId") or data.get("message_id")
            logger.info("sms_sent: phone=%s gateway_id=%s", phone, gateway_id)
            return True, str(gateway_id) if gateway_id else None, None
        except Exception:
            logger.info("sms_sent: phone=%s (response not json)", phone)
